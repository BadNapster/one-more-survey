...
class SurveysRepository {
	...
	async getSurveyAnsweredUsers(surveyID) {
		const userAnswers = await db.sequelize.query(
			'SELECT answer_text, user_id FROM user_answers WHERE question_id = :questionId GROUP BY answer_text, user_id;',
			{
				type: db.QueryType.SELECT,
				replacements: { questionId: surveyID },
			}
		...
		const result = {};
		for (const elem of userAnswers) {
			if (!survey.options.includes(elem.answer_text)) {
				continue;
			}
			// const ans_user = await userRepository.getUserById(elem.user_id);
			const ans_user = await db.User.findByPk(elem.user_id);

			if (!Object.prototype.hasOwnProperty.call(result, elem.answer_text)) {
				result[elem.answer_text] = String(ans_user.name);
			} else {
				result[elem.answer_text] = String(result[elem.answer_text] + ', ' + ans_user.name);
			}
		}
